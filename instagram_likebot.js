...
// var xpath_like_button = "//span[@class='coreSpriteHeartOpen']";
...
function like(resolve, index, max_likes) {
    browser.getCurrentUrl().then(function(url) {
        ...
        browser.findElement(by.className("coreSpriteHeartOpen")).getAttribute('class').then(function(classname) {
            ...
            if (settings.smart_like_mode && (classname.indexOf('whiteoutSpriteHeartFull') > 0)) {
                ...
                // if (classname.indexOf('whiteoutSpriteHeartOpen') > 0) {
                    ...
                    // logger.debug(by.cssSelector("._soakw.coreSpriteHeartOpen"));
                    // browser.findElement(by.cssSelector("._soakw.coreSpriteHeartOpen")).click();
                    // logger.debug(browser.findElement(by.cssSelector(".coreSpriteHeartOpen")).length);
                    // console.log(browser.findElement(by.cssSelector(".coreSpriteHeartOpen")));
                    // browser.find_element_by_css_selector('.coreSpriteHeartOpen').click();

                    // logger.debug('here ');
                    var likeSpan = browser.findElement(by.className("coreSpriteHeartOpen"))
                    var likeAction = likeSpan.findElement(by.xpath(".."));
                    likeSpan.getAttribute('class').then(function(classname) {
                        logger.debug('PARENT CLass '+classname);
                    });
                    likeAction.click();
                    browser.sleep(settings.sleep_delay);
                // }
